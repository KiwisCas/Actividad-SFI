...
io.on('connection', (socket) => {
    ...
    // New: broadcast face changes from any client to all clients with sender id
    socket.on('faceChange', (faceId) => {
        console.log('Received faceChange', faceId, 'from', socket.id);
        // Re-broadcast to everyone including sender so all pages update consistently
        io.emit('faceChange', { faceId, from: socket.id });
    });
